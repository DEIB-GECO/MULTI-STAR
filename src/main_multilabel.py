...
# Feature selection 
if FEAT_SELECT_TYPE == 'RaRe' and DATA_TYPE == 'BRCA':
    selected_feat = pd.read_csv('data/rare_feat_750_BRCA.csv', index_col=0).values
    selected_feat = [elem[0] for elem in selected_feat]
elif FEAT_SELECT_TYPE == 'RaRe' and DATA_TYPE == 'CRIS':
    selected_feat = pd.read_csv('data/cris_rare_feat_1000_CRIS.csv', index_col=0).values
    selected_feat = [elem[0] for elem in selected_feat]

# Load selected features (hybrid)
if FEAT_SELECT_TYPE == 'hybrid':
    if DATA_TYPE == 'CRIS':
...
ML_APPROACHES = {
    'Problem transformation': ['Label Powerset'], # 'Binary Relevance', 'Classifier Chain'
    'Ensemble': ['Classifier Chain', 'RAKEL'],
    'Algorithm adaptation': ['MLkNN', 'MLARAM']
}

MODEL_TYPES = ['XGBoost', 'Logistic Regression', 'Support Vector Machine', 'Random Forest']
# MODEL_TYPES = ['Support Vector Machine']
