...
from utils import log_transform, plot_before_after_counts, NumpyEncoder
from config_model import *
from singlelabel_metrics import *
from rare_feat_selection import sbf_analysis
...
def main(cfg: ProjectConfig):
    ...
    X, _, _, feat_to_keep = remove_extreme(X, change_X = True)
    ...
    elif cfg.train.type_feat_selection == 'rare':
        
        if cfg.train.brca_cris == 'CRIS':
            feat_filename = '/Users/ivamilojkovic/Breast-Cancer-Analysis/data/cris_rare_feat_' + str(cfg.train.num_feat) + '_' + cfg.train.brca_cris + '.csv'
            if os.path.exists(feat_filename):
                selected_feat = pd.read_csv(feat_filename, index_col=0).values
                selected_feat = [elem[0] for elem in selected_feat]
            else:
                # Select N features
                best_feat_model = SelectKBest(score_func=f_classif, k=cfg.train.num_feat) 
                best_feat_model.fit(X_train_scaled, y_train)
                selected_feat = list(X_train_scaled.columns[best_feat_model.get_support()])

                # Select features with RaRe method
                selected_feat = sbf_analysis(X = X_train_scaled[selected_feat], Y = y_train)
                pd.Series(selected_feat).to_csv(
                    '/Users/ivamilojkovic/Breast-Cancer-Analysis/data/cris_rare_feat_' + 
                    str(cfg.train.num_feat) + 
                    '_' + cfg.train.brca_cris + '.csv')
        elif cfg.train.brca_cris == 'BRCA':
            feat_filename = '/Users/ivamilojkovic/Breast-Cancer-Analysis/data/rare_feat_' + str(cfg.train.num_feat) + '_' + cfg.train.brca_cris + '.csv'
            if os.path.exists(feat_filename):
                selected_feat = pd.read_csv(feat_filename, index_col=0).values
                selected_feat = [elem[0] for elem in selected_feat]
            else:
                ...
                selected_feat = sbf_analysis(X = X_train_scaled[selected_feat], Y = y_train)
                pd.Series(selected_feat).to_csv(
                    '/Users/ivamilojkovic/Breast-Cancer-Analysis/data/rare_feat_' + 
                    str(cfg.train.num_feat) + 
                    '_' + cfg.train.brca_cris + '.csv')
    ...
    #                'LightGBM', 'AdaBoost']
    MODEL_TYPES = ['SVC', 'Random Forest', 'XGBoost', 'Logistic Regression']
    # MODEL_TYPES = ['KNN', 'Decision Tree', 'LightGBM', 'AdaBoost']
    ...
    for MODEL_TYPE in MODEL_TYPES:
        ...
        if cfg.train.downsample_test:
            ...
            df_exp_scores = pd.DataFrame(
                columns=['Accuracy', 'MCC', 'Precision - weighted', 'Recall - weighted', 'F1 score - weighted', 'Precision - macro', 'Recall - macro', 'F1 score - macro', 'CV mean', 'CV STD'],
                index=['Train', 'Test/Val'])
            train_scores = list(experiment_params['Train results'].values())
            test_scores = list(experiment_params['Test results'].values())
            df_exp_scores.iloc[0, :] = train_scores[:5] + train_scores[8:11] + [experiment_params['cv_avg_train_scores']] + [experiment_params['cv_std_train_scores']]
            df_exp_scores.iloc[1, :] = test_scores[:5] + test_scores[8:11] + [experiment_params['cv_avg_val_scores']] + [experiment_params['cv_std_val_scores']]
            df_exp_scores.to_csv('exp_params_' + MODEL_TYPE.replace(' ', '') + '_' + experiment_name + '.csv')
